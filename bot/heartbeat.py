...
class Heartbeat:
    ...
    async def _handle_ready(self, me: dict, state: str):
        ...
        try:
            if room_type == "paid":
                game_id, agent_id, ws = await join_paid_game(self.api)
            else:
                game_id, agent_id, ws = await join_free_game(self.api)
        except APIError as e:
            if e.code == "NO_IDENTITY":
                log.error("Identity required. Will setup next cycle.")
                return
            if e.code == "NOT_PRIMARY_AGENT":
                # Per sc-wallet-policy.md: non-primary agents cannot enter rooms.
                # This account is not the primary for its SC wallet.
                # Fall back to free rooms if paid was attempted, otherwise surface error.
                log.error(
                    "❌ NOT_PRIMARY_AGENT — this agent is not the primary for its "
                    "SC wallet. Only the agent with the smallest accounts.id may "
                    "play. See references/sc-wallet-policy.md#primary-agent"
                )
                await asyncio.sleep(30)
                return
            if e.code == "READINESS_BLOCKED":
                log.error("Agent blocked by server: %s. Retrying in 30s.", e.message)
                await asyncio.sleep(30)
                return
    ...
    async def _play_game(self, game_id: str, agent_id: str, entry_type: str, ws=None):
        """Run the WebSocket gameplay engine.

        ws — pre-existing gameplay socket from WsJoinSession (free join path).
             None for paid join or IN_GAME resume; engine will dial /ws/agent.
        """
        ...
        # Run WebSocket engine — pass ws if available (handoff from /ws/join)
