...
async def ensure_whitelist(api: MoltyAPI, owner_eoa: str, agent_eoa: str) -> bool:
    ...
    try:
        ...
        elif e.code == "CONTRACT_WALLET_ALREADY_LINKED":
            # HTTP 409 — this SC wallet is already registered to a different account.
            # Per sc-wallet-policy.md: each SC wallet supports at most one agent.
            # Action: operator must use a different SC wallet or the original account.
            log.error(
                "❌ CONTRACT_WALLET_ALREADY_LINKED (HTTP 409) — "
                "This SC wallet is already linked to another agent account. "
                "Create a new SC wallet or use the original account. "
                "See references/sc-wallet-policy.md"
            )
            return False

        elif e.code == "NOT_PRIMARY_AGENT":
            # HTTP 403 — non-primary agent attempted registration.
            # Per sc-wallet-policy.md: only the agent with the smallest accounts.id
            # for this SC wallet is the primary and may enter rooms.
            log.error(
                "❌ NOT_PRIMARY_AGENT (HTTP 403) — "
                "Only the primary agent (smallest accounts.id for this SC wallet) "
                "may register. See references/sc-wallet-policy.md#primary-agent"
            )
            return False
