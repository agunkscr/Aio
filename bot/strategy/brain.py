...
def decide_action(view: dict, can_act: bool, memory_temp: dict = None) -> dict | None:
    ...
    # ── Priority 2c: Flee from multiple enemies when HP <= 50 ────────
    # If 2+ enemies are in the same region and HP is 50 or below, flee.
    # Does not override Priority 2b (guardian flee already handled above).
    enemies_here = [a for a in visible_agents
                    if not a.get("isGuardian", False) and a.get("isAlive", True)
                    and a.get("regionId") == region_id
                    and a.get("id") != self_data.get("id")]
    if len(enemies_here) >= 2 and hp <= 50 and ep >= move_ep_cost:
        safe = _find_safe_region(connections, danger_ids, view)
        if safe:
            log.warning("⚠️ Outnumbered! %d enemies in region, HP=%d — fleeing to %s",
                        len(enemies_here), hp, safe)
            return {"action": "move", "data": {"regionId": safe},
                    "reason": f"OUTNUMBERED FLEE: {len(enemies_here)} enemies in region, HP={hp}<=50"}
