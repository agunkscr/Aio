"""
Strategy brain v2.0.0 — Full upgrade & bug fix.

Perubahan dari v1.9.2:
─────────────────────────────────────────────────────────────────────
[NEW] Early game looting phase (30 tick pertama / alive > 15)
      - fokus pickup weapon & healing, hindari inisiatif combat
      - movement agresif menuju region berisi item & ruins
      - exit otomatis saat tick > EARLY_GAME_TICKS atau alive turun cepat

[FIX] Pickup fallback tidak lagi menarget item di region lain
[FIX] Healing pickup di-cap saat HP sudah tinggi & stok healing cukup
[FIX] Counter-attack kini butuh HP >= 25 (cegah mati saat harusnya heal)
[FIX] Desperate attack pilih target terlemah, bukan index pertama
[FIX] Endgame hunt (P6) cek favorabilitas damage sebelum engage
[FIX] Guardian approach cek flee-threshold dulu sebelum mendekat
[FIX] _is_enemy_nearby dikurangi jadi 1-hop (2-hop terlalu agresif)
[FIX] Favorable combat (P6b) pertimbangkan projected damage yang diterima
[FIX] alive_count < 30 move bonus dihapus (selalu true, jadi noise)
[NEW] Persistent target tracking (_current_target_id) untuk kill consistency
[NEW] Finish-off priority: musuh low HP mendapat bonus tambahan di target select
[NEW] Inventory capacity diambil dari view (fallback ke 10)
─────────────────────────────────────────────────────────────────────
"""
...
FINISH_OFF_BONUS         = 50   # bonus score untuk kill target yang hampir mati
...
_last_region_id:    str = ""
_current_target_id: str = ""   # [NEW] persistent target untuk kill consistency
...
def reset_game_state():
    ...
    _last_region_id      = ""
    _current_target_id   = ""
    log.info("Strategy brain reset for new game (v2.0.0)")
...
def decide_action(view: dict, can_act: bool, memory_temp: dict = None) -> dict | None:
    ...
    6.   Endgame hunt (alive <= 10, favorability check)
    6b.  Favorable agent combat (projected damage check)
    7.   Monster farming
    ...
    if guardians_all and ep >= 2 and hp >= 35:
        ...
        if _is_in_range(target, region_id, w_range, connections):
            ...
            # [FIX v2.0.0] Cek flee-threshold sebelum approach
    ...
    if alive_count <= 10 and enemies_alive and ep >= 2 and hp >= 20:
        ...
        if _is_in_range(target, region_id, w_range, connections):
            ...
            e_dmg  = calc_damage(target.get("atk", 10),
                                 _estimate_enemy_weapon_bonus(target),
                                 defense, region_weather)
            # [FIX v2.0.0] Favorability check + projected damage
...
def _choose_move_target(connections, danger_ids: set, current_region: dict,
    ...
    for conn in connections:
        ...
        if rid in enemy_regions and alive_count <= 10 and not early_game:
            score += 4
