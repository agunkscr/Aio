"""
Strategy brain v2.1.0 — Looting Hunter Edition.

Perubahan dari v2.0.0:
──────────────────────────────────────────────────────────────────────
[NEW] Looting Hunter (P4c): deteksi player yang sedang looting di region
      berisi item → prioritas serang/kejar sebelum mereka equipped
[NEW] _looting_region_ids: set region yang ada item visible, dipakai di
      scoring movement dan combat target selection
[NEW] LOOTING_HUNTER_BONUS = 80 di _select_best_combat_target
[NEW] Movement bonus besar kalau region berisi PLAYER + ITEM sekaligus
[FIX] Equip loop sniper↔bow (apple-to-apple score comparison)
[FIX] EP drain loop: EP_MOVE_MIN=3, EP_REST_UNTIL=5
[FIX] Anti ping-pong: _recent_regions deque 6 tick, penalti kumulatif
[FIX] EARLY_GAME_MIN_ALIVE: 15→5
[FIX] Early game: cek EP sebelum return move action
[FIX] Early game: supply_cache & medical facility diprioritaskan
[FIX] Early game: self-defense counter saat HP>=40 & diserang
[FIX] Counter-attack: butuh HP>=25
[FIX] Pickup: tidak fallback ke item di region lain
[FIX] Healing score cap saat stok>=3 & HP>=70
[REMOVED] Third-party cleanup (_detect_nearby_fight) → terlalu kompleks
[REMOVED] Binoculars harassment → digabung ke combat block utama
[REMOVED] Endgame camping → bot aktif berburu sampai akhir
[REMOVED] Hold position → ganti dengan EP-based rest yang proper
[REMOVED] Broadcast station dari facility priority
[REMOVED] _is_agent_fighting (hanya dipakai third-party)
──────────────────────────────────────────────────────────────────────
"""
...
FINISH_OFF_HP_THRESHOLD = 25
FINISH_OFF_BONUS        = 70

# [NEW] Looting hunter bonus — player sedang looting jadi target prioritas
LOOTING_HUNTER_BONUS    = 80
PLAYER_BONUS            = 20   # bonus semua player vs guardian
...
EP_MOVE_MIN   = 3   # [FIX] minimum EP sebelum move normal game
EP_REST_UNTIL = 5   # [FIX] rest sampai EP ini sebelum bergerak lagi
...
def reset_game_state():
    ...
    log.info("Brain reset → v2.1.0 Looting Hunter")
...
def decide_action(view: dict, can_act: bool, memory_temp: dict = None) -> dict | None:
    ...
    EG-S3. Self-defense counter (HP>=40, diserang, favorable)
    ...
    5.   Guardian farming (safe approach)
    6.   Endgame hunt (alive<=15)
    6b.  Favorable combat
    7.   Monster farming
            ...
            if ep >= move_ep_cost:
                ...
                if safe:
                    ...
                    return {"action": "move", "data": {"regionId": safe},
                            "reason": f"EARLY: FLEE HP={hp}"}

        # EG-S3: Self-defense (balas kalau diserang & HP aman)
        if _combat_history.get("damage_this_tick") and hp >= 40 and enemies_alive:
    ...
    if looting_players and ep >= 2 and hp >= 25:
        ...
        if target:
            if _is_in_range(target, region_id, w_range, connections):
                ...
                proj   = hits * e_dmg
                # Serang kalau favorable atau musuh hampir mati
                if my_dmg >= e_dmg * 0.8 or target.get("hp", 100) <= my_dmg * 2:
    ...
    if guardians_all and ep >= 2 and hp >= 35:
        ...
        if _is_in_range(tgt, region_id, w_range, connections):
            ...
            if my_dmg >= g_dmg or tgt.get("hp", 100) <= my_dmg * 3:
    ...
    if alive_count <= 15 and enemies_alive and ep >= 2 and hp >= 15:
        ...
        if _is_in_range(tgt, region_id, w_range, connections):
            ...
            proj   = hits * e_dmg
            if my_dmg >= e_dmg * 0.7 or tgt.get("hp", 100) <= FINISH_OFF_HP_THRESHOLD:
    ...
    if enemies_alive and ep >= 2 and hp >= 15:
        ...
        if _is_in_range(tgt, region_id, w_range, connections):
            ...
            hits   = math.ceil(tgt.get("hp", 100) / max(my_dmg, 1))
            proj   = hits * e_dmg
            if my_dmg > e_dmg and hp - proj > 15:
                _current_target_id = tgt.get("id", "")
                return {"action": "attack",
                        "data": {"targetId": tgt["id"], "targetType": "agent"},
                        "reason": f"COMBAT: {my_dmg} vs {e_dmg}"}
            elif tgt.get("hp", 100) <= my_dmg * 2 and hp - proj > 10:
                _current_target_id = tgt.get("id", "")
                return {"action": "attack",
                        "data": {"targetId": tgt["id"], "targetType": "agent"},
                        "reason": f"FINISH OFF: HP={tgt.get('hp')}"}
            elif w_range >= 1 and my_dmg >= e_dmg * 0.75:
                _current_target_id = tgt.get("id", "")
                return {"action": "attack",
                        "data": {"targetId": tgt["id"], "targetType": "agent"},
...
def _choose_move_target(connections, danger_ids, current_region, visible_items,
    ...
    for conn in connections:
        ...
        # [NEW] LOOTING HUNTER movement bonus: ada item + ada enemy = target prioritas
        if has_items and has_enemy and not early_game:
            score += 15   # agresif menuju player yang sedang looting
            log.info("🎯 LOOTING region detected: %s (bonus +15)", rid[:8])
        ...
        if not early_game and alive_count <= 15 and has_enemy and not has_items:
            score += 2   # tetap sedikit tertarik ke enemy region
