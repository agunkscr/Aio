...
# ============================================================================
# COMPATIBILITY LAYER - For existing game loop imports
# ============================================================================
# These wrapper functions allow existing code to work without changes:
...
_global_brain: MoltyRoyaleBrain | None = None
...
def decide_action(view: dict, can_act: bool = True) -> dict | None:
    """
    Compatibility wrapper for decide_action.
    
    Args:
        view: Game state view from WebSocket
        can_act: Whether agent can perform cooldown actions
    
    Returns:
        Action dict: {"action": "...", "data": {...}, "reason": "..."}
        or None if no action needed
    """
    brain = _get_brain()
    return brain.decide_action(view, can_act)


def reset_game_state():
    """
    Compatibility wrapper for reset_game_state.
...
def learn_from_map(view: dict):
    """
    Compatibility wrapper for learn_from_map.
    Update map knowledge from expanded vision.
    
    Args:
        view: Game state view with expanded vision from Map item
    """
    brain = _get_brain()
...
def record_game_result(result: dict):
    """
    Compatibility wrapper for record_game_result.
    Record game end result for learning.
    
    Args:
        result: Game result dict from game_ended message
    """
    brain = _get_brain()
    brain.record_game_result(result)


def get_performance_metrics() -> dict:
    """
    Compatibility wrapper for get_performance_metrics.
    Get performance statistics.
    
    Returns:
        Dict with combat_win_rate, games_played, etc.
    """
    brain = _get_brain()
    return brain.get_performance_metrics()


def get_enemy_profile(agent_id: str) -> dict | None:
    """
    Compatibility wrapper for get_enemy_profile.
    Get profile of specific enemy.
    
    Args:
        agent_id: Enemy agent ID
    
    Returns:
        Enemy profile dict or None if not found
    """
    brain = _get_brain()
    return brain.get_enemy_profile(agent_id)


def clear_all_data():
    """
    Clear all data including learning and enemy profiles.
    Use with caution!
    """
    global _global_brain
    _global_brain = None
    # Also clear the file-based data
    brain = MoltyRoyaleBrain()  # Fresh instance
    brain.clear_all_data()


# ============================================================================
# MAP KNOWLEDGE EXPORT - For websocket_engine.py _update_dz_knowledge
# ============================================================================
# This allows websocket_engine.py to directly update death zone tracking:
...
def get_map_knowledge() -> dict:
    """
    Get map knowledge dict for external updates.
    Returns reference to internal dict so callers can modify it directly.
    
    Returns:
        Dict with keys: death_zones (set), explored (set), facilities (list)
    """
    brain = _get_brain()
    return brain.map_knowledge


# Direct reference for import compatibility
# Usage: from bot.strategy.brain import _map_knowledge
_map_knowledge = get_map_knowledge()
...
# ============================================================================
# 
# Class-based usage (NEW - recommended):
#   from bot.strategy.brain import MoltyRoyaleBrain
#   brain = MoltyRoyaleBrain()
#   decision = brain.decide_action(view, can_act)
#
# Function-based usage (LEGACY - still supported):
#   from bot.strategy.brain import decide_action, reset_game_state, learn_from_map
#   decision = decide_action(view, can_act)
#
# Direct map access (for websocket_engine.py):
#   from bot.strategy.brain import _map_knowledge
#   _map_knowledge["death_zones"].add(region_id)
#
# ============================================================================
