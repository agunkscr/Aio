"""
Strategy brain v1.9.2 — Fix infinite equip loop.

Based on v1.9.1 with the following critical fix:
- _check_equip now returns None if the weapon to equip is already equipped
  (prevents equip-spam loop that blocks healing/combat)
"""
...
def reset_game_state():
    ...
    _last_region_id = ""
    log.info("Strategy brain reset for new game (v1.9.2)")
...
def decide_action(view: dict, can_act: bool, memory_temp: dict = None) -> dict | None:
    """
    Main decision engine v1.9.2.

    Priority chain (unchanged from v1.9.1, fix only in _check_equip):
    1.  DEATHZONE ESCAPE
    ...
    3.  Free actions: pickup (dynamic score), equip (smart swap, anti-loop)
    3b. Utility items
    [cooldown gate]
    ...
    if hp < 20 and not has_healing_items and (enemies_alive or guardians_here):
        if ep >= move_ep_cost:
            ...
            if nearest and _is_in_range(nearest, region_id, w_range, connections):
                my_dmg = calc_damage(atk, get_weapon_bonus(equipped), nearest.get("def", 5), region_weather)
    ...
    if guardians_here and ep >= move_ep_cost:
        ...
        g_dmg = calc_damage(threat_guardian.get("atk", 10),
                            _estimate_enemy_weapon_bonus(threat_guardian), defense, region_weather)
    ...
    # v1.9.2: equip only if different from currently equipped
        ...
        for enemy in enemies_alive:
            if _is_in_range(enemy, region_id, w_range, connections):
                ...
                if my_dmg > e_dmg or enemy.get("hp", 100) <= my_dmg * 2:
                    ...
                    return {"action": "attack", "data": {"targetId": enemy["id"], "targetType": "agent"},
                            "reason": "BINOCULARS RANGED HARASSMENT"}
                break

    # ── Priority 4d: Third‑Party Cleanup ──────────────────────────
    if hp >= 60 and ep >= move_ep_cost + 2 and alive_count > 5:
        nearby_fight = _detect_nearby_fight(recent_logs, my_id, view, connections, danger_ids, region_id, alive_count)
        if nearby_fight:
    ...
    if alive_count <= 5 and _map_knowledge.get("revealed") and region_id in _map_knowledge.get("safe_center", []):
        if not enemies_alive and not guardians_here and ep < max_ep:
            ...
            return {"action": "rest", "data": {}, "reason": f"ENDGAME CAMP: safe centre, EP={ep}/{max_ep}"}

    # ── Priority 8.6: Hold Position ───────────────────────────────
    # If no immediate threat, no items in region, and all neighbours explored, rest
    if not enemies_alive and not guardians_here and ep < max_ep:
        all_explored = all((c if isinstance(c, str) else c.get("id", "")) in _explored_regions for c in connections)
        ...
        if all_explored and no_items_here and not region.get("isDeathZone"):
            log.info("🛑 Hold Position: nothing to do, resting")
            return {"action": "rest", "data": {}, "reason": "HOLD: No incentive to move"}

    # ── Priority 9: Strategic movement ────────────────────────────
    if ep >= move_ep_cost and connections:
        move_target, best_score = _choose_move_target(connections, danger_ids,
                                                       region, visible_items, alive_count,
                                                       enemies_alive, ep)
        # v1.9.1 threshold: only move if score > 0
        if move_target and best_score > 0:
            log.info("🚶 Moving to %s (score=%d)", move_target[:8], best_score)
            _prepare_move(region_id)
...
# ── Helper functions (v1.9.2) ─────────────────────────────────────────

def _check_equip(inventory: list, equipped, view: dict, region_id: str,
                 connections=None) -> dict | None:
    """
    Smart weapon swap with anti-loop fix.
    Returns None if the best weapon is the same as currently equipped.
    """
    nearby_enemy = _is_enemy_nearby(view, region_id, connections)
    # Determine current weapon identity
    current_type = equipped.get("typeId", "").lower() if equipped else "fist"
    current_id   = equipped.get("id", "") if equipped else ""

    best = None
    best_score = get_weapon_bonus(equipped)  # base score is current ATK bonus

    for item in inventory:
        if not isinstance(item, dict) or item.get("category") != "weapon":
            continue
        # [FIX v1.9.2] Skip if this weapon is already equipped
        if item.get("id") == current_id:
            continue
        if item.get("typeId", "").lower() == current_type:
            # Same type, but if it's a different instance (different id) we might still want to keep it
            # For now, skip same type entirely to avoid equip spam
            continue
    ...
    if best:
        ...
        log.info("EQUIP: %s (+%d ATK) chosen (nearby enemy=%s)", best.get("typeId"), new_bonus, nearby_enemy)
...
def _choose_move_target(connections, danger_ids: set, current_region: dict,
    ...
    """
    Returns (region_id, score). score ≤ 0 means no worthwhile move.
    """
    ...
    return candidates[0][0], candidates[0][1]   # return both id and score
...
def _check_pickup(items: list, inventory: list, region_id: str,
    ...
    if len(inventory) >= 10:
        ...
        if drop:
            ...
            return {"action": "drop_item", "data": {"itemId": drop["id"]}, "reason": "MAKE ROOM"}
        return None
    log.info("PICKUP: %s (score=%d)", type_id, score)
    return {"action": "pickup", "data": {"itemId": best["id"]}, "reason": f"PICKUP: {type_id}"}


# ── All other helper functions remain unchanged from v1.9.1 ────────
...
def _use_utility_item(inventory: list, hp: int, ep: int, alive_count: int) -> dict | None:
    ...
    for item in inventory:
        ...
        if type_id == "map":
            ...
            return {"action": "use_item", "data": {"itemId": item_id, "itemType": "map"}, "reason": "UTILITY: Using Map"}
    if alive_count <= 5 and hp > 50 and ep >= MEGAPHONE_MIN_EP:
        for item in inventory:
            ...
            if item.get("typeId", "").lower() == "megaphone":
                ...
                return {"action": "use_item", "data": {"itemId": item["id"], "itemType": "megaphone"}, "reason": "UTILITY: Megaphone lure"}
    return None
...
def learn_from_map(view: dict):
    ...
    log.info("🗺️ MAP LEARNED: %d DZ, top center: %s", len(_map_knowledge["death_zones"]), _map_knowledge["safe_center"][:3])
