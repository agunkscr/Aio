...
def learn_from_map(view: dict):
    """
    Called after Map item is used — agent has expanded vision.
    Updates _map_knowledge with all newly revealed regions:
    - Marks death zones from visibleRegions
    - Identifies safe central regions (high connection count, not DZ)
    - Sets revealed=True so strategy knows full map is available
    """
    global _map_knowledge
    _map_knowledge["revealed"] = True

    visible = view.get("visibleRegions", [])
    safe_candidates = []
    ...
    for region in visible:
        ...
        if region.get("isDeathZone"):
            ...
            # Score by connection count — more connections = more central
            conn_count = len(region.get("connections", []))
            safe_candidates.append((conn_count, rid))

    # Keep top-5 safest central regions for MAP CONTROL guidance
    safe_candidates.sort(reverse=True)
    _map_knowledge["safe_center"] = [rid for _, rid in safe_candidates[:5]]

    log.info(
        "learn_from_map: %d regions scanned, %d DZs known, safe_center=%s",
        len(visible),
