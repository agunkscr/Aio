...
class MoltyAPI:
    ...
    def _handle_response(self, resp) -> dict:
        """Parse response and raise APIError on failure shapes."""
        data = self._safe_parse_json(resp.text)
        if isinstance(data, dict) and not data.get("success", True) and "error" in data:
            err = data["error"]
            raise APIError(
    ...
    async def whitelist_request(self, owner_eoa: str) -> dict:
        ...
        Raises:
            APIError("CONTRACT_WALLET_ALREADY_LINKED", ..., 409)
                SC wallet already linked to another account.
                See references/sc-wallet-policy.md
            APIError("NOT_PRIMARY_AGENT", ..., 403)
                Non-primary agent attempted to register.
        """
        ...
        if resp.status_code == 409:
            ...
            code = err.get("code", "CONTRACT_WALLET_ALREADY_LINKED") if isinstance(err, dict) else "CONTRACT_WALLET_ALREADY_LINKED"
            msg  = err.get("message", "SC wallet already linked to another account") if isinstance(err, dict) else str(err)
            raise APIError(code, msg, 409)

        if resp.status_code == 403:
            data = self._safe_parse_json(resp.text)
            err = data.get("error", {}) if isinstance(data, dict) else {}
            code = err.get("code", "NOT_PRIMARY_AGENT") if isinstance(err, dict) else "NOT_PRIMARY_AGENT"
            msg  = err.get("message", "Only the primary agent may register this wallet") if isinstance(err, dict) else str(err)
