diff --git a/bot/memory/agent_memory.py b/bot/memory/agent_memory.py
index 2a2ba3d6f850e63183785f1f3033d1e6827c6f2f..45b1ec7fbe6fb1ad2215d699d347572c6639097b 100644
    b/bot/memory/agent_memory.py
@@ -1,114 +1,126 @@
 """
 ...
 class AgentMemory:
     ...
     def record_game_end(self, is_winner: bool, final_rank: int,
         ...
         if final_rank is not None:
             current_best = history.get("bestRank")
             if current_best is None or final_rank < current_best:
                 history["bestRank"] = final_rank
 
         history["totalKills"] = history.get("totalKills", 0) + max(0, kills)
         history["totalSmoltz"] = history.get("totalSmoltz", 0) + max(0, smoltz_earned)
         history["winRate"] = round((history["wins"] / history["totalGames"]) * 100, 2)
